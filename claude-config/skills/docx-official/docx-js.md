...
- Values in DXA (twentieths of a point): 1440 = 1 inch
- Apply borders to individual `TableCell` elements, NOT the `Table` itself

**Page-size column width budgets:**
- **Letter** (1" margins) = **9360 DXA** total
- **A4** (1cm margins) = **9600 DXA** total (use 9920 if margins ≤ 1cm; 9600 is conservative)

**Precomputed Column Widths (equal):**
- **Letter 2 cols:** `[4680, 4680]` | **3 cols:** `[3120, 3120, 3120]`
- **A4 2 cols:** `[4800, 4800]` | **3 cols:** `[3200, 3200, 3200]`

**Convert ratios to DXA (helper):**
```javascript
const TOTAL_DXA = 9600; // A4
const ratios = [3, 5, 2]; // any non-negative numbers
const sum = ratios.reduce((a, b) => a + b, 0);
const columnWidths = ratios.map(r => Math.round(r / sum * TOTAL_DXA));
// → [2880, 4800, 1920]
```

**⚠️ Google Docs compatibility (when uploading docx):**
- `columnWidths` is **REQUIRED** — without it Google Docs collapses each column to 1-character width
- Do **NOT** use `TableLayoutType.FIXED` — Google Docs ignores it inconsistently; let docx default behaviour handle layout
- Do **NOT** rely on `HeightRule` for row heights — let content auto-size; Google Docs ignores explicit row heights
- Use `WidthType.DXA` not `WidthType.PERCENTAGE` for cell widths — PERCENTAGE behaves erratically across renderers

**Row-level options:**
- `cantSplit: true` — prevents a row from splitting across pages (use on long rows / KPI tables)
- `tableHeader: true` on the first row — header repeats automatically on each page

**Cell internal padding & vertical centering:**
```javascript
new TableCell({
  width: { size: 4800, type: WidthType.DXA },
  borders: cellBorders,
  margins: { top: 120, bottom: 120, left: 180, right: 180 }, // ~6pt + 9pt — generous, avoids "squashed" look
  verticalAlign: VerticalAlign.CENTER,
  children: [new Paragraph({
    alignment: AlignmentType.LEFT,
    spacing: { before: 40, after: 40, line: 320, lineRule: "auto" }, // 1.6× line height inside cell
    children: [new TextRun({ text: "...", size: 22 })]
  })]
})
```

**Wrap tables with empty paragraphs** to prevent visual collision with surrounding text:
```javascript
const empty = () => new Paragraph({ children: [new TextRun("")], spacing: { after: 60 } });
function pushTable(children, table) {
  children.push(empty(), table, empty());
}
```
...
- **Google Docs upload**: `columnWidths` is non-negotiable (without it columns collapse to 1-char wide); avoid `TableLayoutType.FIXED`, `HeightRule`, and `WidthType.PERCENTAGE` — they render inconsistently
- **A4 budget**: Total content width ≈ 9600 DXA (1cm margins). Use `[4800, 4800]` for 2 cols, `[3200, 3200, 3200]` for 3
- **Wrap tables in empty paragraphs** (`empty()` before + after) to prevent visual collision with surrounding text
- **Row options**: `cantSplit: true` keeps long rows on one page; `tableHeader: true` on first row repeats header on page breaks
- **Cell readability**: `verticalAlign: VerticalAlign.CENTER`, paragraph `spacing: { line: 320, lineRule: "auto" }` (1.6× line height), cell `margins: { top: 120, bottom: 120, left: 180, right: 180 }`